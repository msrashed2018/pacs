...
//	private ActiveDicoms activeDicoms;
	...
	public void handleIncomingFileEvent(NewDicomEvent newDicomEvent) {
		// IMPORTANT! Write everything inside try catch, the guava breaks if an
		// exception occurs

		LOG.info("Incoming Dicom");
		try {
			...
			File dicomFile = newDicomEvent.getFile();// get the file from event handler

			synchronized (dicomService) {
				dicomService.buildAndSaveEntities(dicomFile);// lets build our dicom database entities
			}
