...
public class DicomServer {
	...
	private final class CStoreSCPImpl extends BasicCStoreSCP {
		...
		protected void store(Association as, PresentationContext pc, Attributes rq, PDVInputStream data, Attributes rsp)
			...
			try {
				...
				eventBus.post(new NewDicomEvent(file));

				// let's parse the files
				/*
				 * Attributes attrs = parse(file); if(attrs != null){ String studyiuid =
				 * attrs.getString(Tag.StudyInstanceUID); String patientID =
				 * attrs.getString(Tag.PatientID); patientID = (patientID == null ||
				 * patientID.length() == 0) ? "<UNKNOWN>" : patientID; Long projectID = -1L;
				 * String patientName = attrs.getString(Tag.PatientName); String institutionName
				 * = attrs.getString(Tag.InstitutionName); String uniqueID = file.getName();
				 * Date studyDate = attrs.getDate(Tag.StudyDate); Date studyTime =
				 * attrs.getDate(Tag.StudyTime);
				 * 
				 * String studyDateTime = (studyDate != null && studyTime != null)?new
				 * SimpleDateFormat("MM-dd-yyyy").format(studyDate)+" "+new
				 * SimpleDateFormat("HH-mm-ss").format(studyTime):"01-01-1901 01-01-01";
				 * 
				 * 
				 * 
				 * //eventBus.post(new NewLogEvent(as.toString(), "IMAGE_RECEIVED", ipAddress,
				 * studyDateTime, projectID, patientID, patientName, null, studyiuid,
				 * institutionName, uniqueID)); //eventBus.post(new NewFileEvent(file,
				 * ipAddress, studyiuid, iuid, cuid, associationName));
				 * 
				 * }else{ LOG.error("File Name {} could not be parsed!",file.getName()); }
				 */
	...
	public static DicomServer init(String aeHost, int aePort, String aeTitle, String storageDirectory,
			EventBus eventBus) throws IOException, GeneralSecurityException {
		LOG.info("Starting Dicom Server Bind to: " + aeTitle + "@" + aeHost + ":" + aePort + "; storage: "
				+ storageDirectory);
